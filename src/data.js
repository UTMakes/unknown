...
export const GAME_VERSION = "11.3";
...
export const MILESTONES = [
    { id: 'first_node', name: 'First Steps', desc: 'Place your first node', check: (g) => g.stats.nodesCreated >= 1, reward: { money: 100 } },
    { id: 'network_builder', name: 'Network Builder', desc: 'Have 10 nodes at once', check: (g) => g.nodes.length >= 10, reward: { money: 1000 } },
    { id: 'data_empire', name: 'Data Empire', desc: 'Have 50 nodes at once', check: (g) => g.nodes.length >= 50, reward: { money: 10000, rp: 500 } },
    { id: 'first_million', name: 'First Million', desc: 'Earn $1,000,000 total', check: (g) => g.stats.totalMoney >= 1000000, reward: { money: 100000 } },
    { id: 'researcher', name: 'Researcher', desc: 'Unlock 10 technologies', check: (g) => g.unlocked.length >= 10, reward: { rp: 1000 } },
    { id: 'connected', name: 'Fully Connected', desc: 'Have 25 cables at once', check: (g) => g.conns.length >= 25, reward: { money: 2500 } }
];
...
export const RESOURCES = {
    files: { size: 20, price: 4, rp: 2 },
    images: { size: 80, price: 18, rp: 8 },
    videos: { size: 350, price: 75, rp: 40 },
    audio: { size: 120, price: 28, rp: 15 }
};
...
export const RANDOM_EVENTS = [
    // Good Events
    { id: 'market_boom', name: 'Market Boom', desc: 'Data prices are surging!', type: 'good', duration: 60, effect: (m) => { m.money = 2; }, cleanup: (m) => { m.money = 1; } },
    { id: 'research_grant', name: 'Research Grant', desc: 'Government funding boost!', type: 'good', duration: 45, effect: (m) => { m.rp = 2; }, cleanup: (m) => { m.rp = 1; } },
    { id: 'code_rush', name: 'Code Rush', desc: 'Developers are inspired!', type: 'good', duration: 30, effect: (m) => { m.code = 3; }, cleanup: (m) => { m.code = 1; } },
    { id: 'fiber_upgrade', name: 'Fiber Upgrade', desc: 'ISP upgraded your connection!', type: 'good', duration: 120, effect: (m) => { m.speed = 1.5; }, cleanup: (m) => { m.speed = 1; } },
    { id: 'investment', name: 'Angel Investment', desc: 'An investor believes in you!', type: 'good', instant: true, effect: (g, ui) => { g.money += 5000; ui.showFloat('+ $5,000 (Investment)', window.innerWidth/2, window.innerHeight/2, '#10b981'); } },
    { id: 'bonus_rp', name: 'Research Breakthrough', desc: 'Sudden insight!', type: 'good', instant: true, effect: (g, ui) => { g.rp += 500; ui.showFloat('+ 500 RP (Breakthrough!)', window.innerWidth/2, window.innerHeight/2, '#8b5cf6'); } },
    
    // Bad Events
    { id: 'market_crash', name: 'Market Crash', desc: 'Data prices are plummeting!', type: 'bad', duration: 60, effect: (m) => { m.money = 0.5; }, cleanup: (m) => { m.money = 1; } },
    { id: 'power_outage', name: 'Power Outage', desc: 'Reduced efficiency!', type: 'bad', duration: 30, effect: (m) => { m.speed = 0.5; }, cleanup: (m) => { m.speed = 1; } },
    { id: 'ddos_attack', name: 'DDoS Attack', desc: 'Network under attack!', type: 'bad', duration: 45, effect: (m) => { m.speed = 0.3; }, cleanup: (m) => { m.speed = 1; } },
    { id: 'maintenance', name: 'Emergency Maintenance', desc: 'Servers need repairs!', type: 'bad', instant: true, effect: (g, ui) => { g.money = Math.max(0, g.money - 2000); ui.showFloat('- $2,000 (Maintenance)', window.innerWidth/2, window.innerHeight/2, '#ef4444'); } },
    { id: 'data_breach', name: 'Data Breach', desc: 'Security incident!', type: 'bad', instant: true, effect: (g, ui) => { g.rp = Math.max(0, g.rp - 200); ui.showFloat('- 200 RP (Breach)', window.innerWidth/2, window.innerHeight/2, '#ef4444'); } },
];
